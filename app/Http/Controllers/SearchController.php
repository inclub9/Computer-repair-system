...
class SearchController extends Controller
{
    ...
    public function index()
    {
        ...
        $status = 'ทั้งหมด';
        return view('Searchjob', compact('jobs', 'status'));
    }

        public function FollowJob()
    {
        $tojs = Techonjob::select('job_id')->where('systemuser_id', '=', Auth::user()->id)->orderBy('created_at','desc')->get();
        $jobs = new Collection();
        foreach ($tojs as $toj) {
            $jobs = $jobs->merge(Job::where('id', $toj->job_id)->orderBy('created_at','desc')->get());
        }
        //dd($jobs);
        $status = 'ทั้งหมด';
    ...
    public function SearchFollowJob(Request $request)
    {
        ...
        if (isset($search)){
            if ($selected == 'id') {
                ...
                foreach ($tojs as $toj) {
                    $jobs = $jobs->merge(Job::where($selected, 'like', $req)->where('id', $toj->job_id)->get());
                }
            }
        }else{
            foreach ($tojs as $toj) {
    ...
    public function __construct()
    {
        ...
        $this->middleware('auth:technician')->only('FollowJob');
        $this->middleware('auth:technician')->only('SearchFollowJob');
